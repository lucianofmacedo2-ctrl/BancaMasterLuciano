...
def mostrar_registro(df_csv):
    ...
    with tab_cad:
        ...
        with col_c2:
            nome_novo = st.text_input("Nome (Ex: COLOMBIA 1)", key="nome_cad_s2").strip().upper()
        with col_c3:
    ...
    with tab_exc:
        ...
        with col_e1:
            tipo_exc = st.selectbox("Categoria para Excluir", ["LIGA", "MERCADO", "METODO", "OPERADOR"], key="tipo_exc_s2")
        with col_e2:
            ...
            item_sel_exc = st.selectbox("Selecione para excluir", opcoes_exc if opcoes_exc else ["-"], key="sel_exc_s2")
    ...
    with c_m1:
        fora_da_base = st.checkbox("🚩 Jogo fora da Base de Dados", key="fora_base_s2")
    with c_m2:
    ...
    with col_p1:
        data_m = st.date_input("Data da Aposta", datetime.now(), key="data_reg_s2")
    with col_p2:
        pais_selecionado = st.selectbox("Selecione o País", ["-"] + lista_paises, key="pais_reg_s2")
    with col_p3:
        ...
        liga_selecionada = st.selectbox("Liga", nomes_ligas if nomes_ligas else ["-"], key="liga_reg_s2")

    times_mandantes, times_visitantes = [], []
    ...
    with st.form("form_registro_manual_s2", clear_on_submit=True):
        l2_c1, l2_c2, l2_c3 = st.columns(3)
        with l2_c1: 
            mandante_m = st.text_input("Time Mandante") if fora_da_base else st.selectbox("Time Mandante", ["-"] + times_mandantes)
        with l2_c2: 
            visitante_m = st.text_input("Time Visitante") if fora_da_base else st.selectbox("Time Visitante", ["-"] + times_visitantes)
        with l2_c3: 
    ...
    with st.expander("📤 REGISTRO EM MASSA (CSV) - S2"):
        arquivo_massa = st.file_uploader("Arquivo CSV", type=["csv"], key="uploader_s2")
        if arquivo_massa:
            try:
                ...
                if st.button("🚀 Confirmar Importação CSV", key="btn_csv_s2"):
