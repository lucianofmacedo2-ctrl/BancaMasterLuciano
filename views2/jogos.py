...
def mostrar_jogos(df_hist_input):
    st.title("📅 Agenda & Inteligência - Sistema 2")
    
    brasil_tz = pytz.timezone('America/Sao_Paulo')
    hoje_dt = datetime.now(brasil_tz).date()
    
    if 'data_ex_jogos_2' not in st.session_state:
        st.session_state.data_ex_jogos_2 = hoje_dt.strftime('%d/%m/%Y')
    ...
    for i in range(3):
        if cols_btn[i].button(labels[i], key=f"btn_nav_2_{i}", use_container_width=True):
            st.session_state.data_ex_jogos_2 = datas_ops[i].strftime('%d/%m/%Y')
            st.rerun()

    data_alvo = st.session_state.data_ex_jogos_2[0:5]
    df_dia = df_agenda[df_agenda['Data'].str.contains(data_alvo, na=False)] if not df_agenda.empty else pd.DataFrame()

    if df_dia.empty:
        st.warning(f"Sem jogos mapeados para {st.session_state.data_ex_jogos_2}.")
        return
    ...
    for liga, df_l in df_dia.groupby('Liga'):
        ...
        for idx, row in df_l.iterrows():
            ...
            with c3:
                if st.button("Analisar 🔍", key=f"btn_ana_2_{idx}", use_container_width=True):
                    ...
                    st.rerun()
            with c4:
                if st.button("Simular 🎲", key=f"btn_sim_2_{idx}", use_container_width=True):
    ...
    if times_do_dia:
        st.divider()
        st.subheader(f"📊 Performance dos Times ({st.session_state.data_ex_jogos_2})")
