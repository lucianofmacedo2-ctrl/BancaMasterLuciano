...
def mostrar_simulador(df):
    st.markdown("## 🎲 Simulador de Confrontos - Sistema 2")
    st.write("Projeção de placar, cantos e cartões baseada em médias de ataque e defesa (S2).")
    ...
    if 'liga_simulador_2' in st.session_state:
        matches_l = get_close_matches(st.session_state.liga_simulador_2, lista_ligas, n=1, cutoff=0.6)
        if matches_l:
            idx_liga = lista_ligas.index(matches_l[0])

    # 2. SELEÇÃO DE TIMES
    liga_sel = st.selectbox("🏆 Selecione a Liga (S2)", lista_ligas, index=idx_liga, key="sel_liga_sim_2")
    ...
    if 'time_casa_simulador_2' in st.session_state:
        matches_m = get_close_matches(st.session_state.time_casa_simulador_2, lista_times, n=1, cutoff=0.6)
        if matches_m:
            idx_casa = lista_times.index(matches_m[0])

    col_s1, col_s2 = st.columns(2)
    with col_s1:
        m_sel = st.selectbox("🏠 Mandante (S2)", lista_times, index=idx_casa, key="sel_casa_sim_2")
    
    visitantes_disp = sorted([t for t in lista_times if t != m_sel])
    idx_fora = 0
    if 'time_fora_simulador_2' in st.session_state:
        matches_v = get_close_matches(st.session_state.time_fora_simulador_2, visitantes_disp, n=1, cutoff=0.6)
        if matches_v:
            idx_fora = visitantes_disp.index(matches_v[0])

    with col_s2:
        v_sel = st.selectbox("🚌 Visitante (S2)", visitantes_disp, index=idx_fora, key="sel_fora_sim_2")
    ...
    st.info(f"💡 **Resumo da Projeção S2:** Total Gols FT: {total_exp_gols:.2f} | Total Cantos FT: {total_exp_cantos:.1f} | Total Cartões: {total_exp_cards:.1f}")
