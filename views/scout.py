...
def mostrar_scout(df):
    ...
    # --- 1. FILTROS ---
    c1, c2 = st.columns(2)
    # Ajustado para os nomes exatos das colunas do seu CSV
    ...
    # --- 4. GOLS POR FAIXA DE MINUTOS (TABELA COM DESTAQUE VERDE) ---
    st.divider()
    ...
    def style_faixas(val):
        # Destaca células com incidência acima de 20%
    ...
    with c_f1:
        ...
        st.dataframe(df_faixa_m.style.format("{:.1f}%").applymap(style_faixas), use_container_width=True)
    ...
    with c_f2:
        ...
        st.dataframe(df_faixa_v.style.format("{:.1f}%").applymap(style_faixas), use_container_width=True)
    ...
    def get_stats(dados, col_f, col_s):
        ...
        for nome, serie in [("Feitos", f), ("Sofridos", s)]:
            # Cálculo de Coeficiente de Variação (CV) para medir consistência
