...
# --- CONFIGURAÇÃO DE ESTILO (CENTRALIZAÇÃO) ---
st.markdown("""
    <style>
...
def criar_tabela_estatistica(df_t, time):
    ...
    df_res = pd.DataFrame(data, columns=['Indicador', 'Média', 'Mediana', 'Moda', 'DP', 'CV'])
    return df_res
...
def calcular_incidencia_mercados(df_t, time):
    ...
    rows.append({
        ...
        'HT': f"{df_t['BTTS_HT'].mean()*100:.2f}%",
        'FT': f"{df_t['BTTS_FT'].mean()*100:.2f}%"
    })

    # Cantos
    for c in [7.5, 8.5, 9.5, 10.5]:
        rows.append({
            'Mercado': f'Over {c} Cantos',
            'HT': '-',
            'FT': f"{(df_t['Total_Corners'] > c).mean()*100:.2f}%"
        })
...
def mostrar_scout(df):
    ...
    with t_resumo:
        ...
        render_stat_row("CHUTES NO GOL", get_team_series(df_m, m_sel, 'ShotsOnTarget_H', 'ShotsOnTarget_A').mean(), get_team_series(df_v, v_sel, 'ShotsOnTarget_H', 'ShotsOnTarget_A').mean())

    with t_detalhe:
        format_dict = {c: "{:.2f}" for c in ['Média', 'Mediana', 'Moda', 'DP', 'CV']}
        st.subheader(f"📊 Estatísticas Detalhadas: {m_sel}")
        st.table(criar_tabela_estatistica(df_m, m_sel).style.format(format_dict))
        
        st.subheader(f"📊 Estatísticas Detalhadas: {v_sel}")
        st.table(criar_tabela_estatistica(df_v, v_sel).style.format(format_dict))
