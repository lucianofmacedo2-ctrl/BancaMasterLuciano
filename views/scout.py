...
def calcular_stats_completas(serie):
    """Calcula Média, Mediana, Moda, CV e Desvio Padrão com segurança."""
    if serie.empty or serie.isnull().all():
        return {"Média": 0, "Mediana": 0, "Moda": 0, "DP": 0, "CV%": 0}
    
    # Limpa valores nulos para o cálculo
    s = serie.dropna()
    media = s.mean()
    mediana = s.median()
    try:
...
def formatar_data_seguro(valor):
    """Evita o erro AttributeError se a data for nula."""
    try:
...
def mostrar_scout(df):
    ...
    # Limpeza inicial das colunas para evitar erros de nomes
    ...
    st.divider()
    st.subheader("📈 Estatísticas Detalhadas (Média, Mediana, Moda, DP, CV)")
    ...
    for label, (col_m, col_v) in metricas.items():
        with st.expander(f"📊 {label} (Feitos vs Levados)", expanded=True):
            ...
            with m1:
                ...
                st.dataframe(stats_m.style.format("{:.2f}").background_gradient(cmap="RdYlGn"), use_container_width=True)
            with v1:
                st.write(f"**{v_sel} (Fora)**")
                st.dataframe(stats_v.style.format("{:.2f}").background_gradient(cmap="RdYlGn"), use_container_width=True)

    # --- 4. MINUTOS (SOMATÓRIO E CORES) ---
    st.divider()
