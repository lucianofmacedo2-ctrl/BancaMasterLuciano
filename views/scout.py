...
def mostrar_scout(df):
    ...
    try:
        # Filtros usando as colunas reais do seu CSV
        c1, c2 = st.columns(2)
        ...
        st.divider()
        t1, t2 = st.tabs(["📊 Desempenho", "⏰ Gols por Minuto"])
        ...
        with t2:
            ...
            with c_m:
                ...
                st.dataframe(pd.DataFrame([v_m], columns=faixas, index=["%"]).style.format("{:.1f}%").background_gradient(cmap="Greens", axis=1))
            with c_v:
                st.write(f"**{v_sel}**")
                v_v = [df_v[f"{f}_Visitante"].mean() * 100 for f in faixas]
                st.dataframe(pd.DataFrame([v_v], columns=faixas, index=["%"]).style.format("{:.1f}%").background_gradient(cmap="Greens", axis=1))

    except KeyError as e:
        st.error(f"Coluna não encontrada: {e}. Verifique o arquivo CSV.")
