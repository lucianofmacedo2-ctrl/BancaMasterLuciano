...
def mostrar_dashboard():
    ...
    with c_f2:
        # DICA: Verifique se aqui você selecionou o mês atual (ex: 01/2026) para ver a projeção
    ...
    greens = df_f[df_f['status'].str.contains('Green', na=False)] if not df_f.empty else pd.DataFrame()
    ...
    # --- PROJEÇÃO FINAL (REVISADA) ---
    # Removi a trava de dias_restantes > 0 para que a seção apareça sempre que um mês for selecionado
    if mes_sel != "Todos" and not df_f.empty:
        ...
        pj3.metric("Entradas Estimadas", f"{int(total_apostas + (apostas_por_dia * dias_restantes))} total")
        
        if dias_restantes > 0:
            st.caption(f"Cálculo: {dias_passados} dias ativos + {dias_restantes} dias restantes até o fim do mês.")
        else:
            st.caption("Mês finalizado. A projeção é igual ao resultado atual.")
