...
# --- 2. PROBABILIDADES EXPANDIDAS (O QUE VOCÊ PEDIU AGORA) ---
...
def mostrar_scout(df):
    ...
    # --- TABELA DE PROBABILIDADES EXPANDIDA ---
    ...
    with ce1:
        st.write(f"**{m_sel}**: 🧤 CS: {cs_m} | 🚫 FTS: {fts_m} | 🎯 Chutes p/ Gol: {ch_g_m:.1f}")
    with ce2:
        st.write(f"**{v_sel}**: 🧤 CS: {cs_v} | 🚫 FTS: {fts_v} | 🎯 Chutes p/ Gol: {ch_g_v:.1f}")
    ...
    # --- ESTATÍSTICAS AVANÇADAS (TODAS AS MÉTRICAS ORIGINAIS) ---
    st.divider()
