...
def mostrar_simulador(df):
    ...
    st.write("Projeção avançada baseada em algoritmos de ataque, defesa e distribuição de Poisson.")
    ...
    # 3. CÁLCULO DAS MÉDIAS (Função Expandida)
    def get_team_stats(df_liga, time):
        ...
        # Gols
        gp = pd.concat([df_t[df_t['Mandante'] == time]['Gols_Mandante_FT'], df_t[df_t['Visitante'] == time]['Gols_Visitante_FT']]).astype(float)
        gc = pd.concat([df_t[df_t['Mandante'] == time]['Gols_Visitante_FT'], df_t[df_t['Visitante'] == time]['Gols_Mandante_FT']]).astype(float)
        
        # Consistência (Coeficiente de Variação - Menor é melhor)
    ...
    st.write(f"📊 **Índice de Previsibilidade do Confronto:** {conf_media*100:.1f}%")
    st.progress(conf_media)

    # --- FUNÇÃO FORMATAR PROB ---
    def card_prob(titulo, valor):
        cor = "#28a745" if valor > 60 else "#fd7e14" if valor > 40 else "#dc3545"
        return f"""
            <div class='metric-card'>
                <div class='stat-label'>{titulo}</div>
                <div class='stat-value' style='color: {cor};'>{valor:.1f}%</div>
            </div>
        """

    # --- SEÇÃO GOLS ---
    st.markdown("### ⚽ Análise de Gols (Poisson)")
    total_ht = s_m[2] + s_v[2]
    total_ft = exp_gols_m + exp_gols_v
    
    c1, c2, c3 = st.columns(3)
    c1.markdown(card_prob("Over 0.5 HT", (1 - poisson.cdf(0, total_ht)) * 100), unsafe_allow_html=True)
    c2.markdown(card_prob("Over 1.5 FT", (1 - poisson.cdf(1, total_ft)) * 100), unsafe_allow_html=True)
    c3.markdown(card_prob("Over 2.5 FT", (1 - poisson.cdf(2, total_ft)) * 100), unsafe_allow_html=True)

    # --- SEÇÃO CANTOS E CARTÕES ---
    st.markdown("### 🚩 Cantos e 🟨 Cartões")
    total_cn_ft = (s_m[4] + s_v[5] + s_v[4] + s_m[5]) / 2
    total_cr_ft = (s_m[8] + s_v[9] + s_v[8] + s_m[9]) / 2
    
    c1, c2, c3 = st.columns(3)
    c1.markdown(card_prob("Over 9.5 Cantos", (1 - poisson.cdf(9, total_cn_ft)) * 100), unsafe_allow_html=True)
    c2.markdown(card_prob("Over 10.5 Cantos", (1 - poisson.cdf(10, total_cn_ft)) * 100), unsafe_allow_html=True)
    c3.markdown(card_prob("Over 3.5 Cartões", (1 - poisson.cdf(3, total_cr_ft)) * 100), unsafe_allow_html=True)

    # --- NOVIDADE: PLACARES MAIS PROVÁVEIS ---
    ...
    for i, (placar, prob) in enumerate(placares):
        cols_pl[i].markdown(f"""
            <div style='background:#1f1f1f; color:white; border-radius:10px; padding:10px; text-align:center;'>
                <small>Rank {i+1}</small><br>
                <b style='font-size:20px;'>{placar}</b><br>
                <span style='color:#00ff00;'>{prob:.1f}%</span>
            </div>
        """, unsafe_allow_html=True)

    st.divider()
    st.info(f"💡 **Dica Estratégica:** Este jogo tem um volume projetado de {total_cn_ft:.1f} cantos. Se o Índice de Previsibilidade estiver acima de 70%, as médias tendem a ser muito confiáveis.")
